...
class MainWindow(QMainWindow):
    ...
    def create_download_tab(self):
        ...
        self.last_date_label.setToolTip("Last available date in DB for selected symbol")
        date_layout.addWidget(self.last_date_label)
        self.set_last_date_button = QPushButton("Set to Last+1")
        self.set_last_date_button.setToolTip("Set start date to the day after the last available date for this symbol")
        self.set_last_date_button.clicked.connect(self.set_start_date_to_last_plus_one)
        date_layout.addWidget(self.set_last_date_button)
        today_str = date.today().strftime('%Y-%m-%d')
        self.start_date_input.setText('2020-01-01')  # Will be updated dynamically
        ...
        # Connect symbol/date changes to preview refresh and dynamic start date
        self.symbol_combo.currentIndexChanged.connect(self.update_start_date_dynamic)
        self.symbol_combo.lineEdit().editingFinished.connect(self.update_start_date_dynamic)
...
"""
    ...
    def show_data(self, data):
        ...
        for row_idx, (index, row) in enumerate(data.iterrows()):
            # Defensive: handle 'Date' as column or index
            ...
            self.preview_table.setItem(row_idx, 1, QTableWidgetItem(str(row['Open']) if 'Open' in row else ''))
            self.preview_table.setItem(row_idx, 2, QTableWidgetItem(str(row['High']) if 'High' in row else ''))
            self.preview_table.setItem(row_idx, 3, QTableWidgetItem(str(row['Low']) if 'Low' in row else ''))
            self.preview_table.setItem(row_idx, 4, QTableWidgetItem(str(row['Close']) if 'Close' in row else ''))
            self.preview_table.setItem(row_idx, 5, QTableWidgetItem(str(row['Volume']) if 'Volume' in row else ''))
        # --- Update statistics and chart ---
        if not data.empty:
            stats = data[[c for c in ['Open', 'High', 'Low', 'Close', 'Volume'] if c in data.columns]].agg(['mean', 'median', 'min', 'max', 'std'])
            ...
            if 'Date' in data.columns:
                ...
                x_vals = data_sorted['Date']
            elif data.index.name == 'Date':
                data_sorted = data.sort_index(ascending=True)
                x_vals = data_sorted.index
            else:
                data_sorted = data
                x_vals = range(len(data_sorted))
            if 'Close' in data_sorted.columns:
                ax.plot(x_vals, data_sorted['Close'], label='Close', color='royalblue')
            if symbol:
...
"""
    ...
    def update_start_date_dynamic(self):
        symbol = self.symbol_combo.currentText().strip().upper()
        if not symbol or not symbol.isalpha():
            ...
            return
        try:
            last_date = self.data_reader.get_last_date(symbol)
            if last_date:
                self.last_date_label.setText(f"Last in DB: {last_date}")
                # Set start date to last_date + 1 day
                from datetime import timedelta
                next_date = last_date + timedelta(days=1)
                self.start_date_input.setText(next_date.strftime('%Y-%m-%d'))
            else:
                self.last_date_label.setText("No data in DB")
                self.start_date_input.setText('2020-01-01')
        except Exception as e:
            self.last_date_label.setText("(Error)")
            self.start_date_input.setText('2020-01-01')

    def set_start_date_to_last_plus_one(self):
        symbol = self.symbol_combo.currentText().strip().upper()
        if not symbol or not symbol.isalpha():
            self.start_date_input.setText('2020-01-01')
            return
