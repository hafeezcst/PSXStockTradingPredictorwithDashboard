...
# Import GrokChat
from grok_chat import GrokChat
...
XAI_API_KEY = os.getenv('xai_api_key')  # Changed to match grok_chat.py

# Default model configuration
DEFAULT_MODEL = "Grok-1"  # Changed from "Grok" to match MODEL_OPTIONS key
MODEL_OPTIONS = {
    "Grok-1": {  # Changed from "Grok" to be consistent
        "url": "https://api.x.ai/v1/chat/completions",
        "description": "Best for real-time market analysis and complex reasoning",
        "max_tokens": 4096,
        "model_name": "grok-3-latest",  # Updated to match grok_chat.py
...
def get_api_keys() -> Dict[str, str]:
    """Get API keys from environment variables and session state."""
    keys = {}
    ...
    if 'deepseek_api_key' in st.session_state and st.session_state.deepseek_api_key:
        keys['deepseek'] = st.session_state.deepseek_api_key
    if 'anthropic_api_key' in st.session_state and st.session_state.anthropic_api_key:
        keys['anthropic'] = st.session_state.anthropic_api_key
    if 'huggingface_api_key' in st.session_state and st.session_state.huggingface_api_key:
        keys['huggingface'] = st.session_state.huggingface_api_key
    if 'xai_api_key' in st.session_state and st.session_state.xai_api_key:
        keys['xai'] = st.session_state.xai_api_key
...
def display_financial_reports(config: Dict[str, Any]):
    ...
    if 'analysis_settings' not in st.session_state:
        st.session_state.analysis_settings = {
            "primary_model": DEFAULT_MODEL,  # Use Grok-1 as default
            "use_secondary": False,
            ...
            "temperature": 0.7,
            "max_tokens": MODEL_OPTIONS[DEFAULT_MODEL]["max_tokens"]
        }
    elif 'primary_model' not in st.session_state.analysis_settings:
        # Ensure primary_model is set if analysis_settings exists
        st.session_state.analysis_settings['primary_model'] = DEFAULT_MODEL
...
def display_ai_analysis_tab(df: pd.DataFrame, date_col: str):
    ...
    try:
        ...
        # Add Telegram integration option
        st.sidebar.checkbox("Send analysis to Telegram", key="send_to_telegram")
        ...
        if not company1_reports.empty:
            ...
            if st.button("🔍 Analyze Reports", key="analyze_button", use_container_width=True):
                ...
                selected_model = st.session_state.get('analysis_settings', {}).get('primary_model', 'DeepSeek')
                ...
                if selected_model == "DeepSeek" and (DEEPSEEK_API_KEY or api_keys.get('deepseek')):
                    model_available = True
                elif selected_model == "Claude-3" and (ANTHROPIC_API_KEY or api_keys.get('anthropic')):
                    model_available = True
                elif selected_model == "Grok-1" and (XAI_API_KEY or api_keys.get('xai')):
                    model_available = True
                elif selected_model == "Mistral-7B" and (HUGGINGFACE_API_KEY or api_keys.get('huggingface')):
                    model_available = True
                
                # If selected model is not available, fall back to available models
                if not model_available:
                    model_name = "DeepSeek" if (DEEPSEEK_API_KEY or api_keys.get('deepseek')) else \
                                "Claude-3" if (ANTHROPIC_API_KEY or api_keys.get('anthropic')) else \
                                "Grok-1" if (XAI_API_KEY or api_keys.get('xai')) else \
                                "Mistral-7B" if (HUGGINGFACE_API_KEY or api_keys.get('huggingface')) else "Unknown"
                else:
                    model_name = selected_model
                with st.spinner(f"🤖 {model_name} is analyzing the reports... This may take a few moments."):
                    try:
                        # Get company 1 info
                        ...
                        with analysis_placeholder.container():
                            ...
                            if st.session_state.get('send_to_telegram', False):
                                try:
                                    if send_analysis_to_telegram(analysis, company1_info):
                                        st.success("✅ Analysis sent to Telegram successfully!")
                                    else:
                                        st.warning("⚠️ Failed to send analysis to Telegram. Please check your Telegram configuration.")
                                except Exception as e:
            ...
            # Add cache display at the end of the tab
            display_cached_analyses()
    except Exception as e:
        logger.error(f"Error displaying AI analysis tab: {e}")
        st.error(f"Error displaying AI analysis: {str(e)}")
...
def display_settings_tab():
    ...
    # Display Grok beta notice
    st.warning("""
    ⚠️ **Important Notice: xAI API Access**
    
    The Grok model is currently in beta and requires special access:
    1. Visit [x.ai](https://x.ai/)
    2. Join the waitlist
    3. Once approved, you'll receive instructions to get an API key
    
    **Note:** API access is not guaranteed and is subject to xAI's approval process.
    """)
    ...
    current_model = st.session_state.get('analysis_settings', {}).get('primary_model', DEFAULT_MODEL)
    
    # Find the index of the current model in MODEL_OPTIONS
    ...
    if 'analysis_settings' not in st.session_state:
        st.session_state.analysis_settings = {}
    st.session_state.analysis_settings['primary_model'] = primary_model
    ...
    if model_info['api_type'] == "xai":
        st.markdown("#### 🔑 xAI API Configuration")
        
        # Display beta status and availability note
        st.info("""
        **Grok Model Access Requirements:**
        1. Join the waitlist at [x.ai](https://x.ai/)
        2. Wait for approval email
        3. Follow instructions to get your API key
        4. Enter your API key below
        
        **Note:** If you haven't received approval yet, please use another model.
        """)
        ...
        if current_api_key:
            ...
            api_key = st.text_input(
                ...
                type="password",
                help="Enter your xAI API key. You must join the waitlist and be approved before you can use the API.",
            ...
            if api_key != current_api_key:
                ...
                if api_key:
                    ...
                    if success:
                        ...
                        2. You have joined the waitlist at [x.ai](https://x.ai/)
                        3. Your account has been approved for API access
                        
                        If you haven't joined the waitlist yet:
                        1. Visit [x.ai](https://x.ai/)
                        2. Join the waitlist
                        3. Once approved, you'll receive instructions to get an API key
                        """)
...
def prepare_analysis_prompt(report1_text: str, report2_text: str = None) -> str:
    """
    Prepare the analysis prompt for the AI model.
    
    Args:
        report1_text (str): Text content of the first report
        report2_text (str, optional): Text content of the second report for comparison
    
    Returns:
        str: Formatted prompt for the AI model
    """
    prompt = """Please analyze the following financial report(s) and provide a comprehensive analysis. 
    Focus on key financial metrics, trends, and potential investment implications.
    
    Format your response in the following sections:
    1. SUMMARY: Brief overview of the key findings
    2. KEY METRICS: Important financial metrics and their interpretation
    3. FUNDAMENTAL ANALYSIS: Analysis of company fundamentals
    4. INVESTMENT RECOMMENDATION: Clear investment recommendation with rationale
    ...
    if report2_text:
        ...
        prompt += f"\n{report2_text}\n"
        prompt += "\nPlease provide a comparative analysis of both reports, highlighting similarities, differences, and relative strengths/weaknesses."
    
    prompt += "\n\nPlease provide a detailed analysis following the format specified above."
    return prompt

def split_report(text: str, max_chunk_size: int = 50000) -> List[str]:
    """Split a report into smaller chunks that fit within token limits."""
    if not text:
        return []
    
    # First split by major sections (e.g., Financial Statements, Notes, etc.)
    ...
    for section in major_sections:
        ...
        for para in paragraphs:
            ...
            # If single paragraph is too large, split it into sentences
...
def analyze_financial_reports(report1_text: str, report2_text: str = None, 
                            ...
                            company1_info: Dict = None,
                            company2_info: Dict = None) -> str:
    """Analyze financial reports and return the analysis."""
    try:
        if api_keys is None:
            api_keys = get_api_keys()
        
        # Get the selected model from session state if not provided
        if selected_model is None:
            selected_model = st.session_state.get('analysis_settings', {}).get('primary_model', DEFAULT_MODEL)
        
        # Split reports into smaller chunks with reduced max size
        report1_chunks = split_report(report1_text, max_chunk_size=40000)
        report2_chunks = split_report(report2_text, max_chunk_size=40000) if report2_text else []
        
        logger.info(f"Split report1 into {len(report1_chunks)} chunks")
        if report2_text:
            logger.info(f"Split report2 into {len(report2_chunks)} chunks")
        
        # Prepare the base prompt with minimal size
        base_prompt = """Analyze this financial report section:
1. Key metrics
2. Trends
3. Implications
...
"""
        ...
        if selected_model == "Grok-1" and api_keys.get('xai'):
            try:
                headers = {
                ...
                # Analyze first report chunks
                for i, chunk in enumerate(report1_chunks):
                    logger.info(f"Analyzing chunk {i+1}/{len(report1_chunks)}")
                    prompt = f"{base_prompt}\n\nFirst Report Section {i+1}/{len(report1_chunks)}:\n{chunk}"
                    ...
                    if response.status_code == 200:
                        ...
                        if 'choices' in response_data and len(response_data['choices']) > 0:
                            if 'message' in response_data['choices'][0]:
                                chunk_analysis = response_data['choices'][0]['message'].get('content', '')
                            else:
                                chunk_analysis = response_data['choices'][0].get('text', '')
                            ...
                            if chunk_analysis:
                                ...
                                logger.warning(f"No content received for chunk {i+1}")
                                analyses.append(f"Error: No content received for section {i+1}")
                        else:
                            logger.error(f"Invalid response format from Grok model for chunk {i+1}")
                            analyses.append(f"Error: Invalid response format from Grok model")
                    else:
                        logger.error(f"Error analyzing chunk {i+1}: {response.text}")
                        analyses.append(f"Error analyzing section {i+1}: {response.text}")
                except Exception as e:
                    logger.error(f"Error processing chunk {i+1}: {str(e)}")
                    analyses.append(f"Error processing section {i+1}: {str(e)}")
                
                # Analyze second report chunks if provided
                if report2_chunks:
                    for i, chunk in enumerate(report2_chunks):
                        logger.info(f"Analyzing second report chunk {i+1}/{len(report2_chunks)}")
                        prompt = f"{base_prompt}\n\nSecond Report Section {i+1}/{len(report2_chunks)}:\n{chunk}"
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if 'choices' in response_data and len(response_data['choices']) > 0:
                                    ...
                                    if chunk_analysis:
                                        analyses.append(chunk_analysis)
                                        logger.info(f"Successfully analyzed second report chunk {i+1}")
                                    else:
                                        logger.warning(f"No content received for second report chunk {i+1}")
                                        analyses.append(f"Error: No content received for section {i+1}")
                                else:
                                    logger.error(f"Invalid response format from Grok model for second report chunk {i+1}")
                                    analyses.append(f"Error: Invalid response format from Grok model")
                            else:
                                logger.error(f"Error analyzing second report chunk {i+1}: {response.text}")
                                analyses.append(f"Error analyzing section {i+1}: {response.text}")
                        except Exception as e:
                            logger.error(f"Error processing second report chunk {i+1}: {str(e)}")
                            analyses.append(f"Error processing section {i+1}: {str(e)}")
                
                # Combine all analyses
                if analyses:
                    logger.info(f"Successfully generated {len(analyses)} analyses")
                    combined_analysis = "\n\n".join(analyses)
                    return format_analysis_output(combined_analysis, company1_info, company2_info, used_model)
                else:
                    logger.error("No analyses were generated")
                    return "❌ Error: No analysis was generated. Please try again or select a different model."
                
            except Exception as e:
                logger.error(f"Error with Grok model: {str(e)}")
                return f"""❌ Error with Grok model: {str(e)}
...
3. Using a different model if the issue persists"""
        ...
        # If Grok fails or wasn't selected, try other models
        if not analyses:
            logger.info("Trying alternative models")
            # Try other models in order
            for model in ["DeepSeek", "Claude-3", "Mistral-7B"]:
                if model == "DeepSeek" and (DEEPSEEK_API_KEY or api_keys.get('deepseek')):
                    try:
                        headers = {
                            "Authorization": f"Bearer {DEEPSEEK_API_KEY or api_keys['deepseek']}",
                            "Content-Type": "application/json"
                        }
                        
                        # Analyze first report chunks
                        for i, chunk in enumerate(report1_chunks):
                            logger.info(f"Analyzing chunk {i+1}/{len(report1_chunks)} with DeepSeek")
                            prompt = f"{base_prompt}\n\nFirst Report Section {i+1}/{len(report1_chunks)}:\n{chunk}"
                            ...
                            payload = {
                                ...
                                "max_tokens": MODEL_OPTIONS["DeepSeek"]["max_tokens"],
                                "temperature": 0.7,
                                "top_p": 0.95
                            }
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    chunk_analysis = response.json()['choices'][0]['message']['content']
                                    analyses.append(chunk_analysis)
                                    used_model = "DeepSeek"
                                    logger.info(f"Successfully analyzed chunk {i+1} with DeepSeek")
                                else:
                                    logger.error(f"Error analyzing chunk {i+1} with DeepSeek: {response.text}")
                                    analyses.append(f"Error analyzing section {i+1}: {response.text}")
                            except Exception as e:
                                logger.error(f"Error processing chunk {i+1} with DeepSeek: {str(e)}")
                                analyses.append(f"Error processing section {i+1}: {str(e)}")
                        
                        # Analyze second report chunks if provided
                        if report2_chunks:
                            for i, chunk in enumerate(report2_chunks):
                                logger.info(f"Analyzing second report chunk {i+1}/{len(report2_chunks)} with DeepSeek")
                                prompt = f"{base_prompt}\n\nSecond Report Section {i+1}/{len(report2_chunks)}:\n{chunk}"
                                ...
                                try:
                                    ...
                                    if response.status_code == 200:
                                        chunk_analysis = response.json()['choices'][0]['message']['content']
                                        analyses.append(chunk_analysis)
                                        logger.info(f"Successfully analyzed second report chunk {i+1} with DeepSeek")
                                    else:
                                        logger.error(f"Error analyzing second report chunk {i+1} with DeepSeek: {response.text}")
                                        analyses.append(f"Error analyzing section {i+1}: {response.text}")
                                except Exception as e:
                                    logger.error(f"Error processing second report chunk {i+1} with DeepSeek: {str(e)}")
                                    analyses.append(f"Error processing section {i+1}: {str(e)}")
                        
                        # Combine all analyses
                        if analyses:
                            logger.info(f"Successfully generated {len(analyses)} analyses with DeepSeek")
                            combined_analysis = "\n\n".join(analyses)
                            return format_analysis_output(combined_analysis, company1_info, company2_info, used_model)
                        else:
                            logger.error("No analyses were generated with DeepSeek")
                            continue
                    except Exception as e:
                        logger.error(f"Error with DeepSeek model: {str(e)}")
                        continue
                
                # Similar error handling for other models...
                # (Claude-3 and Mistral-7B implementations follow the same pattern)
            
            # If all models fail, return error message
            if not analyses:
                logger.error("All models failed to generate analysis")
                return f"""❌ Error during analysis: Could not access any AI models.

Please ensure you have configured at least one of the following API keys:
1. DeepSeek API key
2. Anthropic API key for Claude-3
3. xAI API key for Grok
4. Hugging Face API key for Mistral-7B

You can configure these in the Settings tab."""
        
        # Format the analysis
        if analyses:
            logger.info(f"Successfully generated {len(analyses)} analyses")
            combined_analysis = "\n\n".join(analyses)
            formatted_analysis = format_analysis_output(combined_analysis, company1_info, company2_info, used_model)
        ...
        logger.error(f"Error analyzing financial reports: {e}")
        return f"Error analyzing reports: {str(e)}"

def format_analysis_output(analysis: str, company1_info: Dict, company2_info: Dict = None, used_model: str = None) -> str:
    ...
    formatted_analysis += "\n### 🤖 Analysis Details\n"
    if used_model:
        formatted_analysis += f"**Model Used:** {used_model}\n"
        if used_model in MODEL_OPTIONS:
            model_info = MODEL_OPTIONS[used_model]
        ...
        formatted_analysis += "**Model Used:** Unknown\n"
    
    # Add analysis settings
    formatted_analysis += "\n**Analysis Configuration:**\n"
    formatted_analysis += "- Analysis Depth: Detailed\n"
    formatted_analysis += "- Output Format: Concise\n"
...
def display_enhanced_buy_signals(buy_signals):
    """Display enhanced buy signals with better formatting and visualization."""
    try:
        # Create a DataFrame for better display
        signals_data = []
        ...
        if signals_data:
            ...
            # Generate unique identifiers for each element
            unique_id = str(uuid.uuid4())[:8]  # Use first 8 characters of UUID
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            
            # Create tabs for different views
            tab1, tab2 = st.tabs([f"📊 Table View {unique_id}", f"📈 Chart View {unique_id}"])
            ...
            with tab1:
                ...
                st.dataframe(
                    ...
                    key=f"signals_table_{unique_id}"
                )
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"latest_buy_signals_{timestamp}_{unique_id}.csv",
                    mime="text/csv",
                    key=f"download_signals_table_{unique_id}"
                )
            ...
            with tab2:
                ...
                st.plotly_chart(fig, use_container_width=True, key=f"profit_loss_bar_chart_{unique_id}")
                ...
                st.plotly_chart(fig2, use_container_width=True, key=f"holding_days_scatter_chart_{unique_id}")
                
                # Add download button for chart data with unique key
                st.download_button(
                    label="📥 Download Chart Data",
                    data=signals_df.to_csv(index=False),
                    file_name=f"buy_signals_chart_data_{timestamp}_{unique_id}.csv",
                    mime="text/csv",
                    key=f"download_chart_data_{unique_id}"
                )
        else:
            st.info("No valid buy signals to display.")
    except Exception as e:
        logger.error(f"Error displaying buy signals: {e}")
        st.error(f"Error displaying buy signals: {str(e)}")

# Enhanced Analysis Caching System
def get_analysis_cache_path():
    """Get the path to the analysis cache directory."""
    cache_dir = os.path.join(project_root, "data", "cache", "financial_analysis")
    os.makedirs(cache_dir, exist_ok=True)
    return cache_dir

def generate_cache_key(report_content: str, company_info: Dict) -> str:
    """Generate a unique cache key for a report analysis."""
    # Create a unique identifier based on company and report content
    key_content = f"{company_info['symbol']}_{company_info['date']}_{hash(report_content)}"
    return f"analysis_{hash(key_content)}.json"

def get_cached_analysis(report_content: str, company_info: Dict) -> Optional[Dict]:
    """Retrieve cached analysis if available."""
    try:
        cache_key = generate_cache_key(report_content, company_info)
        cache_path = os.path.join(get_analysis_cache_path(), cache_key)
        ...
        if os.path.exists(cache_path):
            ...
            if (datetime.now() - cache_date).days < 30:  # Cache valid for 30 days
                return cached_data['analysis']
    except Exception:
        pass
    return None

def save_analysis_to_cache(report_content: str, company_info: Dict, analysis: str, used_model: str):
...
def get_enhanced_analysis(report1_text: str, report2_text: str = None,
                         company1_info: Dict = None,
                         company2_info: Dict = None) -> str:
    """
    Enhanced analysis function that includes caching.
    This is a wrapper around the original analyze_financial_reports function.
    """
    # Check cache for first report
    cached_analysis1 = get_cached_analysis(report1_text, company1_info) if company1_info else None
    
    # Check cache for second report if provided
    cached_analysis2 = get_cached_analysis(report2_text, company2_info) if report2_text and company2_info else None
    
    if cached_analysis1 and (not report2_text or cached_analysis2):
        # If we have cached analysis for all reports, combine them
        if report2_text and cached_analysis2:
        ...
        return format_analysis_output(cached_analysis1, company1_info, None, "Cached Analysis")
    
    # If no cache available, perform fresh analysis
    analysis = analyze_financial_reports(
        report1_text,
        report2_text,
        company1_info=company1_info,
    ...
    if analysis and not analysis.startswith("❌"):
        if company1_info:
            save_analysis_to_cache(report1_text, company1_info, analysis, "Grok-1")
        if company2_info:
            save_analysis_to_cache(report2_text, company2_info, analysis, "Grok-1")
    
    return analysis

def display_cached_analyses():
    """Display a summary of cached analyses."""
    try:
        cache_dir = get_analysis_cache_path()
        ...
        if cache_data:
            ...
            df['Cached'] = pd.to_datetime(df['Cached'])
            df = df.sort_values('Cached', ascending=False)
            
            # Display cache summary
            st.markdown("### 📚 Cached Analyses")
            st.dataframe(
                df,
                use_container_width=True,
                hide_index=True,
                column_config={
                    ...
                    'Cached': st.column_config.DatetimeColumn("Cached", width="medium")
                }
            )
        ...
        pass  # Silently fail if cache display fails

def send_analysis_to_telegram(analysis_text: str, company_info: dict = None) -> bool:
    """Send financial analysis report to Telegram.
    
    Args:
        analysis_text (str): The analysis text to send
        company_info (dict, optional): Company information for context
    ...
    try:
        ...
        config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'config', 'alert_config.json')
        with open(config_path, 'r') as f:
            config = json.load(f)
        ...
        if not bot_token or not chat_ids:
            logger.error("Telegram configuration missing in alert_config.json")
            return False
        
        # Format the message
        message = "📊 Financial Analysis Report\n\n"
        if company_info:
            message += f"Company: {company_info.get('symbol', 'N/A')}\n"
            message += f"Date: {company_info.get('date', 'N/A')}\n"
            message += f"Report Type: {company_info.get('report_type', 'N/A')}\n\n"
        ...
        # Send each chunk to each chat ID
        success = True
        ...
        if success:
            logger.info("Successfully sent analysis to Telegram")
        return success

    except Exception as e:
        logger.error(f"Error in send_analysis_to_telegram: {e}")
        return False
